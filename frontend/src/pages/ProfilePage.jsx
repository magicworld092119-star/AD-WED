...
export default function ProfilePage() {
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6 pb-12 px-4">
      ...
      {/* Header Banner */}
      <div className="bg-white border border-orange-100 rounded-3xl p-8 flex flex-col md:flex-row items-center justify-between gap-6 shadow-sm relative overflow-hidden">
        <div className="flex items-center space-x-5">
          <div className="w-20 h-20 rounded-2xl bg-orange-100 border border-orange-200 flex items-center justify-center text-orange-600 font-bold text-2xl shadow-sm">
            {user?.fullName ? user.fullName.charAt(0).toUpperCase() : 'U'}
          </div>
          <div className="space-y-1">
            <div className="flex items-center space-x-3">
              <h1 className="text-2xl font-black text-slate-900">{user?.fullName || t('profileTitle')}</h1>
              <span className="px-2.5 py-0.5 rounded-full text-xs font-bold bg-orange-100 text-orange-800 border border-orange-200 flex items-center space-x-1">
                <ShieldCheck className="w-3.5 h-3.5 mr-1 text-orange-600" /> Active Account
              </span>
            </div>
            <p className="text-xs text-slate-600 font-bold">{user?.role || t('profileRoleDefault')} • {user?.hospital || t('profileHospitalDefault')}</p>
      ...
      <form onSubmit={handleSave} className="space-y-6">
        ...
        <div className="bg-white border border-orange-100 rounded-3xl p-8 space-y-6 shadow-sm">
          <h2 className="text-base font-bold text-slate-900 flex items-center space-x-2 border-b border-orange-100 pb-3">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs">
            <div>
              <label className="block text-slate-600 font-bold mb-1">{t('profileFullName')}</label>
              <input
                type="text"
                value={formData.fullName}
                onChange={(e) => setFormData({ ...formData, fullName: e.target.value })}
                placeholder="Enter full name"
                className="w-full px-3 py-2.5 rounded-xl bg-slate-50 border border-slate-200 text-slate-900 focus:outline-none focus:border-orange-500"
              />
            </div>

            <div>
              <label className="block text-slate-600 font-bold mb-1">{t('profileEmail')}</label>
              <input
                type="email"
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs">
            <div>
              <label className="block text-slate-600 font-bold mb-1">{t('profileHospital')}</label>
              <input
                type="text"
            ...
            <div>
              <label className="block text-slate-600 font-bold mb-1">{t('profileRole')}</label>
              <input
                type="text"
                value={formData.role}
